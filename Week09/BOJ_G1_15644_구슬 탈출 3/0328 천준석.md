# BOJ_G1_15644_구슬 탈출3 [2023-03-28]
https://www.acmicpc.net/problem/15644

<접근법>
``` 
0. 구슬 탈출2에서 단순 방향 표시와 저장만 하면 된다.
1. 초기 공백 방향과 방향(0,1,2,3)을 문자로 표시한 내용을 계속 더하면서 마지막에 출력한다.
```
...
while que:
    mini = []
    rx, ry, bx, by, cnt, direct = que.popleft()
    ...
    for i in range(4):
        ...
        # 한 쪽으로 기울었을 때, 빨간 구슬과 파란 구슬이 같은 좌표라면? -> 더 멀리서 온 구슬을 파악해 먼저 온 구슬 바로 전에 위치 시켜야 한다. (핵심인듯?)
            ...
            if visited[move_red_x][move_red_y][move_blue_x][move_blue_y] == 0:
                # 초기 공백 + 움직인 방향 저장
                que.append([move_red_x, move_red_y, move_blue_x, move_blue_y, cnt+1, direct+nd])
